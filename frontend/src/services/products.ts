...
export const productAPI = {
  ...
  create: (data: { name: string; description?: string; price: number; stock: number; kind: ProductKind; category_id?: string; image_url?: string; duration_minutes?: number }): Promise<ProductResponse> =>
    apiClient.post("/products", data).then((res) => res.data),
  update: (
    id: string,
    data: Partial<{ name: string; description: string; price: number; stock: number; image_url: string; duration_minutes: number; kind: ProductKind; category_id: string | null }>
