...
import { Home, ShoppingBag, Calendar, FileText, ShoppingCart, Receipt, BarChart3, Bell, User, PawPrint, Scissors, LogOut, Users, Sun, Moon } from "lucide-react";
  ...
  useEffect(() => {
    const loadNotificationCount = async () => {
      ...
      try {
        ...
        setNotificationCount(notifications.filter((notification) => !notification.is_read).length);
      } catch {
        setNotificationCount(0);
      }
    };
    const onNotificationsUpdated = () => loadNotificationCount();
      ...
      <header className="bg-background border-b sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            {/* Logo */}
            <Link to={isAdmin ? "/reports" : "/"} className="flex items-center gap-2">
              <PawPrint className="w-8 h-8 text-blue-600" />
              <span className="text-xl font-semibold text-foreground">petty</span>
            </Link>

            {/* Navigation */}
            <nav className="hidden md:flex items-center gap-6">
              {!isAdmin && (
                <Link to="/" className={`flex items-center gap-2 px-3 py-2 rounded-lg transition ${location.pathname === '/' ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:text-gray-900'}`}>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex gap-6">
              ...
              <aside className="hidden lg:block w-64 flex-shrink-0">
                <div className="bg-card text-card-foreground rounded-lg border p-4 sticky top-24">
                  <nav className="space-y-1">
                    ...
                    <Link to="/shop" className={`flex items-center gap-3 px-3 py-2 rounded-lg transition ${isActive('/shop') ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50'}`}>
      ...
      <footer className="bg-background border-t mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-semibold text-gray-900">petty</span>
              </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-gray-600">
                <li><Link to="/support" className="hover:text-gray-900">Support Center</Link></li>
                <li><a href="mailto:support@petcare.com" className="hover:text-gray-900">Contact Us</a></li>
          ...
          <div className="border-t mt-8 pt-8 text-center text-sm text-gray-600">
            <p>&copy; 2026 petty. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
