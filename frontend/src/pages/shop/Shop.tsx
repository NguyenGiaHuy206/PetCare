...
import { ShoppingCart, Star, Search, Loader2, Plus, Trash2, Edit, Upload } from "lucide-react";
...
export default function Shop() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const data = await productAPI.getAll(0, 50, search || undefined, "shop");
        setProducts(data);
      } catch (err: unknown) {
  ...
  const filteredProducts = products.filter((product) =>
    category === "all" ? true : product.category_id === category
  );
  ...
  const handleAddToCart = async (product: ProductResponse) => {
    try {
      ...
      setError(getApiErrorMessage(err, "Failed to add item to cart. Please try again."));
    } finally {
        ...
        {user?.role === "admin" && showForm && (
          <form onSubmit={handleSaveProduct} className="bg-white rounded-lg border p-6 mb-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input value={formName} onChange={(e) => setFormName(e.target.value)} required placeholder="Product name" className="px-3 py-2 border border-gray-300 rounded-lg" />
              <select value={formCategoryId} onChange={(e) => setFormCategoryId(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg">
                <option value="">Select category</option>
                {categories.map((cat) => (
                  <option key={cat.id} value={cat.id}>{cat.name}</option>
                ))}
              </select>
              <input value={formPrice} onChange={(e) => setFormPrice(e.target.value)} required type="number" step="0.01" placeholder="Price" className="px-3 py-2 border border-gray-300 rounded-lg" />
              <input value={formStock} onChange={(e) => setFormStock(e.target.value)} required type="number" placeholder="Stock" className="px-3 py-2 border border-gray-300 rounded-lg" />
            </div>
