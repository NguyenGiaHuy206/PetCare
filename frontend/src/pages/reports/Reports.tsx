...
export default function Reports() {
  ...
  const isRevenueOrder = (order: OrderResponse) => ["paid", "cod_pending"].includes(order.payment_status.toLowerCase());
  ...
  const topProducts = Object.values(
    revenueOrders.flatMap((order) =>
      order.items.filter((item) => item.product_kind !== "service").map((item) => {
  ...
  const handleExport = () => {
    const blob = createXlsxBlob([
      ...
      },
      {
        name: "Orders",
        rows: [
          ["Order ID", "User ID", "Fulfillment Status", "Payment Method", "Payment Status", "Total (VND)", "Created At", "Items"],
          ...filteredOrders.map((order) => [
            ...
            order.total,
            order.created_at,
            order.items.map((item) => `${item.product_name ?? item.product_id} x ${item.quantity}`).join(", "),
          ]),
        ],
      },
      {
        name: "Bookings",
        rows: [
          ["Booking ID", "User ID", "Pet", "Service", "Status", "Datetime", "Duration"],
          ...filteredBookings.map((booking) => [
            booking.id,
            booking.user_id,
            booking.pet_name ?? booking.pet_id,
            booking.service,
            booking.status,
      ...
      },
      {
        name: "Top Products",
        rows: [
          ["Product ID", "Product", "Category", "Units Sold", "Revenue (VND)"],
          ...topProducts.map((item) => [item.productId, item.product, item.category, item.units, item.revenue]),
        ],
      },
      {
        name: "Services",
        rows: [["Service", "Bookings"], ...serviceData.map((item) => [item.name, item.value])],
      },
    ]);
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `petty-report-${new Date().toISOString().slice(0, 10)}.xlsx`;
    link.click();
            ...
            <tbody>
              {topProducts.map((item, idx) => (
                <tr key={idx} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-right font-medium text-gray-900">{formatVnd(item.revenue)}</td>
                </tr>
              ))}
