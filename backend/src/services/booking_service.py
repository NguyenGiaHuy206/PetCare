...
class BookingServiceImpl:
    ...
    async def create(
        ...
        if booking_datetime.tzinfo is None:
            ...
            booking_datetime = booking_datetime.astimezone(timezone.utc)
        ...
        await NotificationService(self.db).create(
            ...
            "booking",
            "Booking created",
            f"Your {service} booking for {pet.name} has been created. Please complete payment to secure it.",
        )
        await NotificationService(self.db).create_for_admins(
            "booking",
            "New service booking",
            f"{pet.name} has a new {service} booking request.",
        )
    ...
    async def update_status(
        ...
        # FSM validation: admins can transition to any state, users follow strict rules
        if is_admin:
            # Admins can transition to any state
            pass
        else:
            # FSM validation for regular users
            valid_transitions = {
    ...
    async def update_notes(
        self, booking_id: uuid.UUID, user_id: uuid.UUID, notes: str, is_admin: bool = False
    ) -> Booking:
    ...
    async def delete(self, booking_id: uuid.UUID, user_id: uuid.UUID, is_admin: bool = False) -> bool:
        """Delete a booking."""
        booking = await self.repo.get_by_id(booking_id)
        if not booking:
            raise ValueError("Booking not found")
