...
class BookingRepository:
    ...
    async def get_by_id(self, booking_id: uuid.UUID) -> Booking | None:
        ...
        result = await self.db.execute(
            select(Booking).options(selectinload(Booking.pet)).where(Booking.id == booking_id)
        )
    ...
    async def get_by_pet(self, pet_id: uuid.UUID) -> list[Booking]:
        ...
        result = await self.db.execute(
            select(Booking).options(selectinload(Booking.pet)).where(Booking.pet_id == pet_id).order_by(
    ...
    async def get_all(self, skip: int = 0, limit: int = 20, status: str | None = None) -> list[Booking]:
        ...
        query = select(Booking).options(selectinload(Booking.pet))
        if status:
