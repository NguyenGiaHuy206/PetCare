...
from src.domain.care_log_service import CareLogService
...
async def update_care_log(
    ...
    try:
        ...
        log = await service.update(uuid.UUID(log_id), current_user.id, **update_data)
        return CareLogResponse.model_validate(log)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
...
async def delete_care_log(
    ...
    service = CareLogService(db)
    try:
        await service.delete(uuid.UUID(log_id), current_user.id)
